...
export default function BottomNav() {
    ...
    return (
        <nav className="fixed bottom-0 left-0 right-0 z-[999] bg-gradient-to-b from-green-800 to-green-950 border-t border-green-700/50 lg:hidden pb-[env(safe-area-inset-bottom)] shadow-[0_-4px_20px_rgba(30,58,138,0.3)]">
            <div className="flex items-center justify-around h-16">
                <Link
                    href="/"
                    className={`flex flex-col items-center justify-center flex-1 h-full relative transition-colors ${
                        isActive('/') ? 'text-white font-bold' : 'text-green-200/70 hover:text-green-100'
                    }`}
                >
                    <Search className="w-5.5 h-5.5" />
                    <span className="text-[10px] font-bold mt-1 uppercase tracking-wider">Telusuri</span>
                    {isActive('/') && (
                        <div className="absolute top-0 w-12 h-1 bg-green-300 rounded-b-full" />
                    )}
                </Link>
                ...
                <Link
                    href="/wishlist"
                    className={`flex flex-col items-center justify-center flex-1 h-full relative transition-colors ${
                        isActive('/wishlist') ? 'text-white font-bold' : 'text-green-200/70 hover:text-green-100'
                    }`}
                >
                ...
                <Link
                    href={getAccountLink()}
                    className={`flex flex-col items-center justify-center flex-1 h-full relative transition-colors ${
                        isAccountActive() ? 'text-white font-bold' : 'text-green-200/70 hover:text-green-100'
                    }`}
                >
                    {admin ? (
